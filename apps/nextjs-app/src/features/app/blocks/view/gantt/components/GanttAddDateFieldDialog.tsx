...
export const AddDateFieldDialog = () => {
  ...
  const { startDateField, endDateField } = useGantt();
  ...
  const onClick = async () => {
    ...
    const startDateField = await createField({
      tableId,
      fieldRo: {
        name: t('table:gantt.dialog.startDate'),
        type: FieldType.Date,
        options: {
    ...
    const endDateField = await createField({
      tableId,
      fieldRo: {
        name: t('table:gantt.dialog.endDate'),
        type: FieldType.Date,
        options: {
          formatting: defaultFormatting,
        },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle>{t('table:gantt.dialog.addDateField')}</DialogTitle>
        </DialogHeader>

        <div className="py-1">{t('table:gantt.dialog.content')}</div>

        <DialogFooter>
          <DialogClose asChild>
            <Button size="sm" type="button" variant="ghost">
              {t('table:gantt.dialog.notAdd')}
            </Button>
          </DialogClose>
          <Button size="sm" onClick={onClick}>
            {t('table:gantt.dialog.addDateField')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
